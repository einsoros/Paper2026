...
| **진행** | 🔴 **단계 D — A-3·C-1 backbone 종료, 기존 pool 9편 소진(생존 0).** 제한적 신규 baseline 탐색이 다음 |
...
## 2. 🔴 현재 진행 작업 — 단계 D · 🔴 backbone 없음 · 신규 baseline 탐색 필요
...
> 🟢 **단계 C 종료 (2026-09-11)** — 확정표 `24-` §17 · 종료 판정 §18-3. 원전검증 11편
> 🔴 **단계 D 진행 중 (2026-09-12) — backbone이 없다** → `25-stageD-baseline-termination`
> 🔴 **A-3 종료** — X·Z(LMX·소통 빈도)가 **협업 생산구조 변수가 아니다** + 원칙 3 세 척도 모두 🟡 → `25-` §1-1
> 🔴 **C-1 종료** — 상위 질문에 맞는 Y(전반적 성과평정)에서 **X→Y가 개별 검정으로 지지되지 않았다**(t = 1.44, r²pb = .03) + **contribution-identification difficulty가 검증된 M이 아니다** → `25-` §1-2. ⚠️ **저자는 omnibus로 가설 지지를 보고했다 — 우리가 더 엄격한 선별 기준을 쓴 것이다.** 🔴 **「비유의 = 관계 없음」으로 쓰지 않는다**
> 🔴 **유의한 Y(보너스 배분·몰입 지각)로 갈아타지 않는다** — 연구범위가 바뀐다 → `25-` §1-3
> ⭕ **상위 질문 / 연구전략을 분리했다** — 상위 질문 **「Human-AI 협업이라는 새로운 생산조건에서 개인의 성과를 어떻게 이해할 수 있는가」** · 연구전략 **「기존 H-H의 검증된 관계를 baseline으로 삼아 H-AI에서 유지·변화를 검토한다」**. 🔴 **섞지 않는다. B는 A를 좁히는 선택이며 B가 실패해도 A는 남는다** → `25-` §2
> 🔴 **평가 정확성은 현재 상위 질문에 넣지 않는다** — 다른 층위의 후속 질문. ⚠️ **확보 문헌에서 참값이 확보된 적이 사실상 없다** → `25-` §2-2
> ⬜ **미해결 — `00-` §2 압축형에 「정확하게」가 남아 있어 위 결정과 충돌한다.** 처리 방식 둘 중 택일해야 한다 → `25-` §2-3
> ⭕ **고속 필터 F1~F4 확정 (2026-09-12) — 🔴 더 수정하지 않는다** → `25-` §3. **F1 범위 적합성 → F2 검증 관계 → F3 H-AI 재관찰 이유 → F4 실행가능성**
> 🔴 **기존 baseline pool 9편 소진 — 생존 0** → `25-` §4. **F1에서 7편 탈락**(Y가 팀 성과·노력·자기보고·자원배분이거나 X가 협업이 아님). 🟡 D-1(Mitchell & Liden 1982)만 원전 미확보로 남는다
> ⚠️ **pool 자체가 F3을 고려하지 않고 만들어졌다.** 🔴 **「생존자 없음」을 「적절한 baseline이 없다」나 research gap으로 쓰지 않는다**
> 🔴 **다음 — ① `00-` §2 압축형 처리 → ② 제한적 신규 baseline 탐색**(겨냥점: **협업이 X이면서 타인의 개인 성과 판단이 Y인** 교집합. **탐색 종료규칙을 먼저 정한다**) **→ ③ 문서 정비** → `25-` §7
> 🔴 **`22-`의 생존 판정은 무효, `23-`은 지위 강등, `21-` 규칙 2와 단계 D 정의는 갱신 대상** → `25-` §5
> 🔴 **단계 D는 시작하지 않는다**
...
| **24-stageC-humanai-change-factors** | **단계 C — 확정표 §17 · 종료 판정 §18** |
| ⭐ **25-stageD-baseline-termination** | **단계 D — A-3·C-1 종료 · 상위질문/전략 분리 · 고속 필터 F1~F4 · 기존 pool 소진** |
...
2-PlanC/
  ...
  25-stageD-baseline-termination.md    ⭐ 단계 D backbone 종료·필터
  99-HANDOFF.md
