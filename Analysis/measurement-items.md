...
version: "[V9] MOD 정의 갱신 — B안 확정, 문항 재설계는 6단계 이연"
...
**[V2] Manipulation check로 전환 확정:**
[V2] 결정에 따라 IV는 시나리오 조건 더미변수(고결합/저결합)로 대체됨. 아래 5문항은 manipulation check 용도로 전환되어, 시나리오 조작이 의도한 대로 협업 결합도를 지각하게 했는지 확인하는 데 사용된다.
...
**[V1] 응답자 시점 통일:**
본 연구의 실험·설문 설계상 응답자는 "평가 대상자(평가받는 구성원)" 시점으로 통일됨. 모든 문항은 응답자 본인이 협업 상황에서 지각하는 귀속 모호성을 측정한다.
...
## Appraisal Procedure Clarity (AI 활용 성과평가 절차 명확성)
Moderator measure. Scale anchor: 1 = 전혀 그렇지 않다, 7 = 매우 그렇다

**[V9] MOD 재정의 — B안 확정:**
조절변수는 "AI 활용 성과평가 절차 명확성"으로 재정의된다. 이는 "조직이 AI 활용 성과를 어떻게 평가하는지 절차를 명확히 설명하는 정도"를 뜻하며, 평가 대상자가 지각하는 조직의 절차적 구조 차원이다.

기존 A안("평가자가 인간/AI 기여를 분리하는 기준을 제시")은 MV(귀속 모호성)와 개념적으로 근접하여 매개조절 모형의 정당성을 훼손할 위험이 있었다. B안은 MV(평가 대상자의 인지 상태)와 MOD(조직의 절차적 구조 지각)를 차원적으로 분리한다.

⚠️ 문항 수준 재설계는 6단계(설문 설계 구체화)에서 처리. 현재 아래 문항은 A안 기준으로 작성된 초안이므로 B안 정의에 맞게 전면 재작성 필요.

| 번호 | 문항 (⚠️ 6단계 재설계 예정) | 이론적 근거 |
| --- | --- | --- |
...
**[V9] 명칭 갱신:** 응답자 시점 통일에 따라 "Evaluator AI literacy" → "Respondent AI literacy (응답자/평가 대상자의 AI 리터러시)"로 갱신.
...
- [V1] 응답자 시점 통일: MV 문항 6개 전체를 평가 대상자 시점으로 통일
- [V2] IV 섹션 → manipulation check로 전환 확정
- [V6] Evaluation Acceptance 강건성 검정 활용 계획 명시
- [V9] MOD 섹션 헤더 및 정의 갱신 — B안(AI 활용 성과평가 절차 명확성) 확정, 문항 재설계 6단계 이연 플래그 추가; AI Literacy 명칭 Evaluator → Respondent 갱신
