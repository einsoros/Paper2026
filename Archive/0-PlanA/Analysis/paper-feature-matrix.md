...
| Colquitt (2001) | 조직공정성의 차원 구조와 측정 도구 부재 | CFA, SEM, 2개 표본 | 대학생 301명, 제조업 직원 337명 | 조직공정성은 4요인으로 구분되며 각각 독립적 예측타당도를 가진다 | 4요인 구조 우수. 절차공정성 척도 검증 | 자기보고식 단일출처 편향 | DV(절차공정성) 측정 — consistency·bias suppression 사용(accuracy 제외). ✅ **[08-16 정정] DV3이 Table 1(p.389) item 4에 대응.** 「기준 재작성」 표기는 부정확했다. Layer 3 | high |
...
| Hartmann & Slapničar (2012) ✅ 원문 대조 | 성과평가 특성 4개가 절차공정성 지각에 미치는 영향과 불확실성의 조절 | 설문 실증 (은행업 178명, PLS 하위집단 비교) | Management Accounting Research | **과업 불확실성(TU)·모호성 내인성(TFA)이 조절.** 네 특성(형식성·발언·결과지표·지표다양성)이 독립변수, 절차공정성이 종속변수 | 🔴 **매개 미검증.** TU–절차공정성 **상관 r=.222 (p<.01)** 은 있으나 경로 아님 | 단일 업종 · TU는 과업 층 조작화(Withey et al. 1983 분석가능성) · **본 연구 MV(평가 귀속 층)와 구성개념 상이** | 🔴 **H2 앵커 아님 — 역할 재배치.** ① 2장 선행연구 ② **MOD 논거**(FORM은 저TU에서만 유의, 전체표본 n.s.) ③ **기여 논거**(불확실성을 외생 조절로 놓은 선례) | high |
